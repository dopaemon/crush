...
func TestHasVerifierCommandEvidence(t *testing.T) {
	msgs := []message.Message{
		{
			Parts: []message.ContentPart{
				message.ToolResult{
	...
	require.True(t, hasVerifierCommandEvidence(msgs))
}

func TestBuildRuntimeSystemGuidance_VerifierPassWithoutEvidence(t *testing.T) {
	agentTools := []fantasy.AgentTool{
		&mockAgentTool{name: AgentToolName},
	}
